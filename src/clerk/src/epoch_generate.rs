...
pub async fn make_epoch_update() -> EpochUpdate {
    ...
    let epoch_sk = std::fs::read(Path::new("data/epoch.sk"))
    ...
    let ticket_pk_in = std::fs::read(&Path::new("data/ticket.pk"))
    ...
    let mint_pk_in = std::fs::read(&Path::new("../mint/data/mint.pk"))
        .expect("Please generate Mint keys before running!");
    eed.mint_signing_keys.push(mint_pk_in);

    // Now load in a TTC gpk
    let ttc_gpk = std::fs::read(&Path::new("../registrar/data/wallet.ttc.gpk"))
