...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let nac_sk = std::fs::read(&Path::new("../registrar/data/wallet.nac.sk")).unwrap();
    let nac_cred = std::fs::read(&Path::new("../registrar/data/wallet.nac.cred")).unwrap();
    let nac_gpk = std::fs::read(&Path::new("../registrar/data/wallet.nac.gpk")).unwrap();

    // Load in the TTC credential so we can create randomized credentials.
    let ttc_gpk = std::fs::read(&Path::new("../registrar/data/wallet.ttc.gpk")).unwrap();
    let ttc_cred = std::fs::read(&Path::new("../registrar/data/wallet.ttc.cred")).unwrap();
    ...
    for (index, signed_ticket) in msg.tickets.iter().enumerate() {
        std::fs::write(
            Path::new(&format!("data/ticket.{}.pb", index)),
